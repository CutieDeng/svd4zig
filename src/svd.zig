...
pub const Register = struct {
    ...
    pub fn format(self: Self, comptime _: []const u8, _: std.fmt.FormatOptions, out_stream: anytype) !void {
        ...
        const sort_alg = std.sort.pdq;
        sort_alg(Field, self.fields.items, {}, fieldsSortCompare); 
...
pub const Field = struct {
    ...
    pub fn fieldResetValue(bit_start: u32, bit_width: u32, reg_reset_value: u32) u32 {
        ...
        const bit_start_u5 : u5 = @intCast(bit_start); 
        const shifted_reset_value = reg_reset_value >> bit_start_u5; 
        const bit_width_u6 : u6 = @intCast(bit_width); 
        const reset_value_mask : u32 = @intCast((@as(u33, 1) << bit_width_u6 ) - 1);
